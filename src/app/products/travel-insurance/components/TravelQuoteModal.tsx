...
import { TravelPlan } from "./TravelPlansGrid";
...
export default function TravelQuoteModal({ plan, initialDestination, onClose }: TravelQuoteModalProps) {
  const [formData, setFormData] = useState({
    fullName: "",
    phone: "",
    email: "",
    destination: initialDestination || "Schengen Area (Europe)",
    duration: "14 Days",
    travelers: "1 Adult (Age 18-40)"
  });
      ...
      <div className="bg-white rounded-3xl max-w-lg w-full p-6 sm:p-8 shadow-2xl border border-zinc-200 relative max-h-[90vh] overflow-y-auto">
        ...
        {quoteSuccess ? (
          <div className="py-8 text-center space-y-4">
            ...
            <h3 className="text-2xl font-black text-zinc-900">Trip Covered!</h3>
            <p className="text-sm text-zinc-600 max-w-sm mx-auto">
              Our travel insurance specialist for <strong>{plan.insurer}</strong> has sent your visa-compliant quote and instant payment link to your email & WhatsApp.
            </p>
          </div>
        ) : (
          <div className="space-y-5">
            <div>
              <span className="text-[10px] font-bold text-teal-600 uppercase tracking-widest">
                VISA-READY TRAVEL COVER
              </span>
              <h3 className="text-xl font-extrabold text-zinc-900 tracking-tight mt-1">
                {plan.name}
              </h3>
              <p className="text-xs text-zinc-500 mt-1">
                Medical Cover: <strong className="text-teal-600 font-bold">{plan.medicalCoverage}</strong> | Starting from ₹{plan.startingPrice}/trip
              </p>
            </div>
            ...
            <form onSubmit={handleSubmit} className="space-y-3.5">
              ...
              <div>
                <label className="block text-xs font-bold text-zinc-700 uppercase tracking-wider mb-1">
                  Email Address (for instant policy delivery)
                </label>
                <input
                  type="email"
